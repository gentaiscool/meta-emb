...
class TransformerTagger(nn.Module):
    ...
    def __init__(self, emb_size, hidden_size, num_layers, num_heads, dim_key, dim_value, filter_size, max_length, input_dropout, dropout, attn_dropout, relu_dropout, word2id, id2word, label2id, id2label, 
                cuda=True, pad_idx=0, use_crf=False, emb_list=[], mode="attn_sum", no_projection=False):
        ...
        if use_crf:
            self.output_layer = CRFOutputLayer(hidden_size, len(label2id))
        else:
    ...
    def forward(self, word_src, word_tgt, raw_inputs, src_lengths=None, print_loss=True):
        """
        ...
        if print_loss:
            loss_val = self.output_layer.loss(hidden, word_tgt)
            return output, loss_val, attn_scores
        else:
            return output, None, attn_scores