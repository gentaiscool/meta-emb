...
----------------------------------------------------------------------------------
Modified on Feb 10, 2018

This script has been modified to run the CALCS shared task on NER.

Change log [Feb 10, 2018]:
- Compatible with Python 3 
- Update on named entity types
...
calcs_bio  = ('B', 'I', 'O')
calcs_tags = ('PER', 'ORG', 'LOC', 'GROUP', 'PROD', 'TITLE', 'EVENT', 'TIME','OTHER')
...
def make_tok(word, bio_tag, sent_id=-1, word_id=-1):
    ...
    Raises:
        ValueError
    """
    if bio_tag == 'O' or len(bio_tag.split('-',1)) == 1:
        bio, tag = 'O', 'O'
    else:
        arr = bio_tag.split('-', 1)
        bio = arr[0]
        tag = arr[1]
        if bio not in calcs_bio or tag not in calcs_tags:
            if verbose:
...
def toks_to_entities(toks, verbose=False):
    """
    Args:
...
def measure(lines, verbose=False):
    """
    ...
    if verbose:
        ...
        results["accuracy"] = (100. * results['tokens'].correct / results['tokens'].gold)
        results["precision"] = (100. * results['tagged'].p)
        results["recall"] = (100. * results['tagged'].r)
        results["fb1"] = (100. * results['tagged'].f)
